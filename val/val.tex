...
Most similar to our work is context-conditioned reinforcement learning with imagined goals (CCRIG), which learns a conditional variational auto-encoder (CVAE)~\citep{sohn2015cvae} that generates goals conditional on the current scene \citep{nair2019ccrig}, as covered in chapter~\ref{chapter:ccrig}. CCRIG was able to learn pushing skills that generalized mainly to object color and partially to object geometry. Our work extends CCRIG in a number of ways. First, we learn expressive generative models that are able to generate goals in scenes with significantly more visual diversity. Second, we learn a diverse set of skills (e.g., grasping, drawer opening, object placing) that require the goal generation to understand affordances of the environment. Finally, we show that we can use off-policy RL on prior experience, in addition to fine-tuning further on a single specific task to learn new skills. These differences allow our method to better operate in real-world scenarios, as borne out in our experiments.
